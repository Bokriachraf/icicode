...
/* Champ verrouillé — même look mais non éditable */
const readonlyClass =
  'w-full rounded-lg px-4 py-3 text-[15px] transition cursor-not-allowed'
...
/* ─────────────────────────────────────────────────────────
   Extraire nom / prenom / email depuis userInfo
   Gère deux cas :
     1. Register classique  → userInfo.name = "Prénom Nom"
     2. Google OAuth        → userInfo.name peut être le nom
                              complet, userInfo.email fourni
   ──────────────────────────────────────────────────────── */
function parseUserInfo(userInfo) {
  if (!userInfo) return { nom: '', prenom: '', email: '' }

  const email = userInfo.email || ''

  // Si le backend stocke déjà nom/prenom séparément
  ...
  // Sinon on découpe userInfo.name (ex: "Ahmed Ben Ali")
...
export default function Step3({ onPrevious }) {
  ...
  const inscriptionData = useSelector((s) => s.inscription?.inscriptionData) || {}
  const userInfo        = useSelector((s) => s.userSignin?.userInfo)         || null
  const { niveaux = [] }= useSelector((s) => s.niveauList || {})

  /* Données identité extraites du compte connecté — immuables */
  const identity = useMemo(() => parseUserInfo(userInfo), [userInfo])
  ...
  const handleSubmit = async (e) => {
    ...
    /* On fusionne identity (depuis le compte) + form (saisie) */
    const finalData = {
      ...inscriptionData,
      ...identity,
      ...form,
    ...
    dispatch(setInscriptionData({ ...identity, ...form, niveauId }))
    setLoading(true)
    setMessage(null)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Bannière info — données verrouillées */}
      <div
      ...
      >
        ...
        <span>
          Ces informations sont récupérées depuis votre compte.
          Pour les modifier, rendez-vous dans vos{' '}
          <strong style={{ color: 'var(--brand-cyan)' }}>paramètres de profil</strong>.
        </span>
      </div>
      ...
      <div>
        ...
        {niveauChoisi && (
          <div className="mt-3 flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm border"
            style={{ background: 'rgba(0,255,209,0.08)', borderColor: 'rgba(0,255,209,0.3)', color: 'var(--brand-cyan)' }}>
      ...
      {message && (
        <p className="text-sm pt-1" style={{ color: '#FF8A8A' }}>{message}</p>
      )}
    </form>
  )
}
